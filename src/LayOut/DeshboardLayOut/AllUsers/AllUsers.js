...
const AllUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://laptop-corner-server.vercel.app/users');
    ...
    const haldlemakeAdmin = id => {
        fetch(`https://laptop-corner-server.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
    ...
    const handleDeleteUser = (user) => {
        fetch(`https://laptop-corner-server.vercel.app/users/${user._id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
